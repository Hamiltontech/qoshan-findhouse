...
useEffect(()=>{
  let arr = data.sort((a, b) => new Date(b?.x_studio_create_date_wp) - new Date(a?.x_studio_create_date_wp));
  setProperty(arr)
}, [])
...
const keywordHandler = (item) => {
  if (item?.x_name?.toString()?.toLowerCase().includes("قيد الإنشاء".toLowerCase())) {
    return (
      item?.x_name
    )
  }
  ...
  return (
    <>
      ...
      <Slider {...settings} arrows={true}>
        {data?.filter(keywordHandler)?.slice(0, 12)?.map((item) => (
